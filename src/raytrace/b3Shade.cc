...
void b3Scene::b3Illuminate(
	...
	b3_surface *surface)
{
	result->r += surface->diffuse.r * light->m_Color.r;
	result->g += surface->diffuse.g * light->m_Color.g;
	result->b += surface->diffuse.b * light->m_Color.b;
	...
	b3_count     depth_count)
{
	...
	if (b3Intersect(ray))
	{
		...
		{
			light = (b3Light *)item;
			b3Illuminate(light,&ray->color,&surface);
		}
