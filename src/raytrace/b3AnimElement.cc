	...
	b3_f64       t)
{
	...
	t1 = b3Math::b3Round(std::clamp(t - ANIM_STEP, m_Start, m_End), epsilon);
	t2 = b3Math::b3Round(std::clamp(t + ANIM_STEP, m_Start, m_End), epsilon);
	if (t1 != t2)
	{
	...
	if (Anim->m_Flags & ANIMFLAGF_LIGHT)
	{
		...
		if (Light != nullptr)
		{
			...
			Anim->b3GetPosition(&Light->m_Position,
				std::clamp(t, Anim->m_Start, Anim->m_End));
