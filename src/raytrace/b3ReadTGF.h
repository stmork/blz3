...
class b3TGFReader
{
	...
	b3Scene *b3Parse(char *buffer,b3_size size,const char *filename);
	b3_bool  b3ParseCamera(b3Scene *scene,char *ptr);
	b3_bool  b3ParseLight(b3Scene *scene,char *ptr);
	b3_bool  b3ParseMaterial(char *tr);
	b3_bool  b3ParseGeometry(b3Scene *scene,char *ptr);
